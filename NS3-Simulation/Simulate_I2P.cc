...
class I2PHeader : public Header
{
    ...
    public:
        ...
        {
            static TypeId tid = TypeId ("I2PHeader")
                                .SetParent<Header> ()
                                .AddConstructor<I2PHeader> ()
                                ;
            return tid;
        }
        ...
        uint32_t Deserialize (Buffer::Iterator start)
        {
            ...
            return 21; // the number of bytes consumed.
        }
    ...
    private:
        uint8_t FPFlag[4];  //1 - Forward
        uint32_t forwardAddress[4];
        uint8_t hopCount;
};
...
int main (int argc, char *argv[])
{
    ...
    for(int it=1;it<=numberOfTestIteration;it++) {
        for(int jt=1;jt<=repeatDataLength;jt++) {
            ...
            int min = 0;
            int range = max - min;
            uint32_t inputTunnelLength = 2;
            uint32_t outputTunnelLength = 2;
            // uint32_t path[4];
            uint32_t outputTunnel[3];
            uint32_t inputTunnel[3];

            int firstHop = rand() % range + min;
            outputTunnel[0]=firstHop;

            for(i=0;i<outputTunnelLength;i++){
                int nextHop = rand() % range + min;
                while(true) {
                    bool isExist = false;
                    for(int j=0;j<i+1;j++)
                        if(nextHop == outputTunnel[j])
                            isExist = true;
                ...
                outputTunnel[i+1]=nextHop;
            }

            for(i=0;i<inputTunnelLength+1;i++){
                int nextHop = rand() % range + min;
                while(true) {
                    bool isExist = false;
                    for(int j=0;j<i;j++)
                        if(nextHop == inputTunnel[j])
                            isExist = true;
            ...
            printf("\nPath (sender-output-tuunel --> receiver-input-tunel):\n");
            printf("N%d -> N%d -> N%d  ------> N%d -> N%d -> N%d\n", 
                        outputTunnel[0],outputTunnel[1],outputTunnel[2],
                        inputTunnel[0],inputTunnel[1],inputTunnel[2]
            );
            for(i = 0; i < outputTunnelLength + inputTunnelLength; ++i)
            {
                keys[i] = (rand() % 100000000000) + 1;
                packetParameters.flags[i] = 1;
            }

            std::cout << "############ --> []" << addresses[inputTunnel[2]] << std::endl;
            packetParameters.addresses[0] = addresses[outputTunnel[2]].Get() - keys[0];
            packetParameters.addresses[1] = addresses[inputTunnel[0]].Get() - keys[1];
            packetParameters.addresses[2] = addresses[inputTunnel[1]].Get() - keys[2];
            packetParameters.addresses[3] = addresses[inputTunnel[2]].Get() - keys[3];
            ...
            for(i = 0; i < 12; ++i){
                packetParameters.packetData[i] -= (keys[0] + keys[1] + keys[2] + keys[3]);
            }
            packetParameters.packetData[12] = '\0';
            packetParameters.hopCount = 0;

            Simulator::Schedule(Seconds(1), &SendStuff, nSocket[outputTunnel[0]], addresses[outputTunnel[1]], port, packetParameters);
            Simulator::Run();
        }
    }
...
void SendStuff (Ptr<Socket> sock, Ipv4Address dstaddr, uint16_t port, struct parameters packetParameters)
{
    ...
    for(i = 0; i < 4; ++i)
    {
        i2pHeader.SetFlag(packetParameters.flags[i], i);
        if(packetParameters.flags[i] == 1)
            i2pHeader.SetForwardAddress(packetParameters.addresses[i], i);
        else
            i2pHeader.SetForwardAddress(0, i);
    }
    i2pHeader.SetHopCount(packetParameters.hopCount);

    p->AddHeader(i2pHeader);


    NS_LOG_INFO("\n" << iaddr.GetIpv4() << " Sending packet to: " << dstaddr);

    int hopCount = (int) (i2pHeader.GetHopCount());
    if(hopCount == 0) {
        std::cout << "[*] Hop-Count: " << (int)(i2pHeader.GetHopCount()) << "\n";
        // sentTime = Simulator::Now().GetSeconds();
        // std::cout << "delay " << sentTime << "\n";
...
void socketRecv (Ptr<Socket> socket)
{
    ...
    I2PHeader receivedI2PHeader;
    packet->RemoveHeader(receivedI2PHeader);
    NS_LOG_INFO("" << receivedI2PHeader);
    
    int hopCount = (int) (receivedI2PHeader.GetHopCount());
    if(hopCount == 4) {
        std::cout << "[*] Hop-Count: " << (int)(receivedI2PHeader.GetHopCount()) << "\n";
    ...
    for(i = 0; i < 12; ++i){
        packetParameters.packetData[i] += keys[receivedI2PHeader.GetHopCount()];
    }

    tempFlags = receivedI2PHeader.GetFlags();
    tempAddr = receivedI2PHeader.GetForwardAddresses();

    NS_LOG_INFO("" << "Decrypted Forward Address: " << Ipv4Address(tempAddr[0] + keys[receivedI2PHeader.GetHopCount()]));
    ...
    if(tempFlags[0] == 1)
    {
        ...
        packetParameters.flags[3] = 0;
        packetParameters.addresses[3] = 0;
        packetParameters.hopCount = receivedI2PHeader.GetHopCount() + 1;
        SendStuff (socket, Ipv4Address (tempAddr[0] + keys[receivedI2PHeader.GetHopCount()]), port, packetParameters);
    }
...
void Create2DPlotFile()
{
  ...
  std::string plotTitle               = "Message delay in I2P";
  std::string dataTitle               = "I2P";
